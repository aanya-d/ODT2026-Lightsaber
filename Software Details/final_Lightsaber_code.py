#frankencode pt 2- combined LDR + neopixel + push button + web server with gyro data
...
#neopixel, ldr, pb, buzzer, initialization
...
body{background:#000;color:#FFE81F;font-family:'Orbitron',Georgia,serif;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:20px;overflow:hidden}
.stars{position:fixed;top:0;left:0;width:100%;height:100%;z-index:0;pointer-events:none}
.star{position:absolute;background:#fff;border-radius:50%}
.wrap{position:relative;z-index:1;width:100%;max-width:480px;text-align:center}
.ep{font-size:11px;letter-spacing:6px;color:#aaa;margin-bottom:8px;text-transform:uppercase;font-family:'Orbitron',serif}
h1{font-size:clamp(36px,10vw,72px);font-weight:900;letter-spacing:4px;color:#FFE81F;text-shadow:0 0 30px #FFE81F88;line-height:1;margin-bottom:6px;font-family:'Orbitron',serif}
.sub{font-size:11px;letter-spacing:8px;color:#aaa;margin-bottom:28px;font-family:'Orbitron',serif}
...
.panel h2{font-size:11px;letter-spacing:4px;color:#FFE81F;margin-bottom:14px;text-align:center;text-transform:uppercase;font-family:'Orbitron',serif}
.panel ul{list-style:none;padding:0}
.panel ul li{font-size:13px;color:#ccc;padding:7px 0;border-bottom:1px solid #FFE81F22;line-height:1.5;display:flex;align-items:flex-start;gap:10px;font-family:'Orbitron',serif}
.panel ul li:last-child{border-bottom:none}
.dot{color:#FFE81F;font-size:16px;line-height:1.4;flex-shrink:0}
.btn{display:inline-block;background:#FFE81F;color:#000;font-family:'Orbitron',Georgia,serif;font-size:18px;font-weight:900;letter-spacing:6px;text-transform:uppercase;padding:16px 48px;border-radius:2px;text-decoration:none;border:none;cursor:pointer;transition:background .2s}
...
<div class="wrap">
  ...
  <div class="panel">
    ...
    <ul>
      ...
      <li><span class="dot">&#9670;</span><span>Tilt your wrist to move the pointer and hit targets</span></li>
...
<div id="hud">
  ...
  <div style="text-align:center"><div class="score-label">Time</div><div class="score-val" id="tm">90</div></div>
...
function updateHUD(){
  ...
  if(timeLeft<=10){el.style.color='#f44';}
  else if(timeLeft<=30){el.style.color='#ffaa00';}
  var lstr='';
...
function loop(ts){
  ...
  if(delta>=1000){
    ...
    timeLeft=Math.max(0,timeLeft-secs);
  ...
  // ── POINTER MOVEMENT ──────────────────────────────────────────────
  var rawX, rawY;
  if(Math.abs(gyroY) > DEADZONE){ rawX = gyroY;  }
  else                           { rawX = 0; smoothX = 0; }

  if(Math.abs(gyroX) > DEADZONE){ rawY = -gyroX; }
...
function endGame(reason){
  ...
  if(reason==='time'){
    document.getElementById('ot').textContent="TIME'S UP";
    document.getElementById('op').textContent='90 seconds have elapsed';
  } else {
...
#main loop - hardware + broswer game
while True:
    ldr_val = ldr.value()

    if ldr_val == 1 and not system_on:
        print("darkness detected, system on")
        for i in range(0, 61):
            np[i] = (100, 100, 100)
            np.write()
            time.sleep(0.04)
    ...
    try:
        ...
        pass  # no incoming connection this cycle, continue hardware loop
